...
class ProfilePicCubit extends Cubit<ProfilePicState> {
  ...
  final FirebaseStorageService _storage = injection.get();
  ...
  updateUser(BuildContext context) async {
    ...
    if (pickedFile != null) {
      ...
      String? url;
      (await _updateUserUseCase.call(UpdatePPParams(user.email, imageFile.path)))
          .fold((l) => null, (r) => url = r.photoUrl);
      emit(ProfilePicSuccess(url!));
    }
  }
  ...
  getProfilePic() async {
    String? url;
    emit(ProfilePicLoading());
    (await _getUserUsecase.call(user.email)).fold((l) => null, (r) => url = r.photoUrl);
    emit(ProfilePicSuccess(url!));
  }
}
